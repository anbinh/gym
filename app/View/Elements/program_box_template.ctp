	...
	<div 
	    ...
	    dnd-disable-if="isSelected == false"       
	 	class="user_favorite_exercise_img" ng-class="{exercise_box_highlight: $first}" style="background-color:{{item.Program.color_code}};">
		    ...
		    <div style="text-align:center;"><a href="/Programs/program_view/{{item.Program.id}}"><img ng-class="(item.Program.is_public == 1)? 'img_program' : (($first)? 'img_program_private_first' :'img_program_private')" src="/upload/image/{{item.Program.photo}}"></a></div>
